...
def writer(filename, row):
    ...
    if uc == filename:
        ...
        if len(row[3]) <= 30000:
            filename.write("<field name=\"data\">"+row[3].decode('utf-8').replace('<','&lt;').replace(">",'&gt;').replace("&",'&amp;').replace("\"",'&quot;').replace("\'",'&apos;').replace("&quot;", "")+"</field>\n")
    else:
        ...
        if len(row[3]) <= 30000:
            filename.write("<field name=\"encode_data\">"+encode_row.decode('utf-8').replace('<','&lt;').replace(">",'&gt;').replace("&",'&amp;').replace("\"",'&quot;').replace("\'",'&apos;').replace("&quot;", "").replace("&amp;", "")+"</field>\n")
            filename.write("<field name=\"data\">"+row[3].decode('utf-8').replace('<','&lt;').replace(">",'&gt;').replace("&",'&amp;').replace("\"",'&quot;').replace("\'",'&apos;').replace("&quot;", "").replace("&amp;", "")+"</field>\n")
...
all_ =[_2gram_list,_3gram_list,_4gram_list,_5gram_list,_6gram_list,uc_list,]
            ...
            if row[0] not in list_:
                ...
                if count == 2000:
                    ...
                    if index == 0:
                        ...
                        _2gram.close()
                        _2gram = codecs.open("../../data/birth_xml/birth_2gram"+str(file_count)+".xml","w",'utf-8')
                    if index == 1:
                        finish_writer(_3gram)
                        _3gram.close()
                        _3gram = codecs.open("../../data/birth_xml/birth_3gram"+str(file_count)+".xml","w",'utf-8')
                    if index == 2:
                        finish_writer(_4gram)
                        _4gram.close()
                        _4gram = codecs.open("../../data/birth_xml/birth_4gram"+str(file_count)+".xml","w",'utf-8')
                    if index == 3:
                        finish_writer(_5gram)
                        _5gram.close()
                        _5gram = codecs.open("../../data/birth_xml/birth_5gram"+str(file_count)+".xml","w",'utf-8')
                    if index == 4:
                        finish_writer(_6gram)
                        _6gram.close()
                        _6gram = codecs.open("../../data/birth_xml/birth_6gram"+str(file_count)+".xml","w",'utf-8')
                    if index == 5:
                        finish_writer(uc)
                        uc.close()
                        uc = codecs.open("../../data/birth_xml/birth_uc"+str(file_count)+".xml","w",'utf-8')

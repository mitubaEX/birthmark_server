...
def solr_serchclass(birthmark, first_class, secound_class):
    ...
    if len(str(first_class)) <=5000:
        ...
        url = url_query+"&sort=strdist(value,\""+url_query_tmp+"\",edit)+desc&rows=100&fl=*,value,lev:strdist(value,\""+url_query_tmp+"\",edit)&wt=python&indent=true"
            ...
            if len(class_line) >= 2:
                ...
                if class_line[0] != "":
                    ...
                    if "cvfv" in param[0]:
                        ...
                        j = "wsp"
                    result = commands.getoutput("java -jar ~/birthmark_server/stigmata/target/stigmata-5.0-SNAPSHOT.jar -b "+str(j)+" extract ~/birthmark_server/data/jar/"+first_class.replace(".","/")+".class > ~/Desktop/tmp/"+first_class+".csv")
                    print result
                    result = commands.getoutput("python ~/birthmark_server/2015scis_yamamoto/prog/fuzzyhashing.py -b ~/Desktop/tmp/"+first_class+".csv").split("\n")
                    for i in result:
                        if i != "":
                            i = i.split(" ")
                            solr_serchclass(j, str(i[1]), secound_class)
